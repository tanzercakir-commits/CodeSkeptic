...
def validate(document, code, version):
    ...
    require(document["version"] == "2.1.0" and len(document["runs"]) == 1, "invalid SARIF run")
    run = document["runs"][0]
    ...
    require(len(run["invocations"]) == 1, "missing or ambiguous invocation")
    ...
    require(total == blocking + report_only == number(report["total"]) == len(run["results"]), "finding count mismatch")
    actual_blocking = 0
    for finding in run["results"]:
        blocks = finding["properties"]["codeskeptic/blocksVerdict"]
    ...
    require(number(coverage["skipped_tus"]) == broken and recovery <= attempted, "coverage alias/count mismatch")
