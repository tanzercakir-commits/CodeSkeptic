...
This appends newly merged protected-main transitions to the generated,
append-only [progress ledger](docs/PROGRESS.md) and refreshes only the marked
state block in [TODO](docs/TODO.md). Never write a `MERGED` progress entry or
edit generated bytes by hand: phase branches, local test output, changelog
prose, and AI statements are not completion authority. The read-only docs-sync
gate reconstructs the expected ledger from git and fails on stale, rewritten,
or non-ancestor state. Detailed rationale and measurement receipts continue to
belong in [the changelog](docs/devlog/changelog.md).
