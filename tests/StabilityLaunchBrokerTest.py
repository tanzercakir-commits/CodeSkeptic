...
class StabilityLaunchBrokerTest(unittest.TestCase):
    ...
    def test_broker_explicitly_requires_kernel_pollrdhup(self) -> None:
        ...
        class Poller:
            def register(self, descriptor: int, events: int) -> None:
                registrations.append((descriptor, events))

            def poll(self, timeout: int):
                timeouts.append(timeout)
                return [(server.fileno(), self_outer.broker.select.POLLRDHUP)]
        ...
        try:
            with mock.patch.object(
                ...
                self.broker._require_peer_half_close(server, 0.05)
            poll_factory.assert_called_once_with()
            self.assertEqual(timeouts, [50])
            self.assertEqual(registrations[0][0], server.fileno())
            self.assertTrue(
                registrations[0][1] & self.broker.select.POLLRDHUP
            )
        finally:
            client.close()
            server.close()
        ...
        try:
            ...
            client.sendmsg(
                ...
                [(
                    socket.SOL_SOCKET,
                    socket.SCM_RIGHTS,
                    struct.pack("ii", read_fd, write_fd),
                )],
            )
            client.shutdown(socket.SHUT_WR)
            with self.assertRaises(self.broker.BrokerError):
...
class StabilityLaunchClientTest(unittest.TestCase):
    ...
    def test_client_sends_nothing_and_accepts_bounded_fragmented_result(self) -> None:
        account = pwd.getpwuid(os.getuid())
        with tempfile.TemporaryDirectory() as temporary:
            ...
            class FragmentingSocket:
                ...
                def recvmsg(
                    self,
                    buffer_size: int,
                    ancillary_size: int = 0,
                    flags: int = 0,
                ):
                    result = self.wrapped.recvmsg(
                        min(buffer_size, 7), ancillary_size, flags
                    )
                    received_sizes.append(len(result[0]))
                    return result
            ...
            try:
                with mock.patch.object(
                    self.client.socket,
                    "socket",
                    side_effect=lambda *args, **kwargs: FragmentingSocket(
            ...
            self.assertGreater(len([size for size in received_sizes if size]), 1)
            self.assertEqual(received_sizes[-1], 0)
    ...
    def test_client_rejects_wrong_mode_trailing_and_ancillary_results(self) -> None:
        ...
        def exercise(sender, error_pattern: str | None = None) -> None:
            with tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                root.chmod(0o755)
                path = root / "campaign.sock"
                ...
                try:
                    expected_error = (
                        self.assertRaisesRegex(
                            self.client.ClientError, error_pattern
                        )
                        if error_pattern is not None
                        else self.assertRaises(self.client.ClientError)
                    )
                    with expected_error:
                        self.client.run_client(
                            "campaign",
        ...
        self.assertEqual(self.client.MAX_RESULT_BYTES, 128)
        exercise(
            lambda connection: connection.sendall(b"x" * 129),
            "launch result framing drift",
        )
...
class StabilityLaunchUnitTest(unittest.TestCase):
    def test_socket_templates_are_distinct_group_bound_stream_endpoints(self) -> None:
        expected = (
