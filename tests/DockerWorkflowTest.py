...
class DockerWorkflowTest(unittest.TestCase):
    def test_one_time_legacy_cleanup_is_absent_after_receipt(self) -> None:
        workflow = WORKFLOW.read_text(encoding="utf-8")

        self.assertNotIn("Remove exact legacy v0.4.9 package version", workflow)
        self.assertNotIn("LEGACY_TAG:", workflow)
        self.assertNotIn("LEGACY_DIGEST:", workflow)
