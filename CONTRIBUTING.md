...
Work only on the first full task in [FIFO TODO](docs/TODO.md), on its own
`agent/cs3-chdd-sdd-uddd-slug` branch. Start with:

```bash
python3 -B scripts/project_queue.py check
```

Use the task's focused test budget, then exact-head independent read-only
verification. The primary prepares a one-task POP, commits only its three
ledger files, and runs the transition guard. No manual TODO/PROGRESS edits,
phase-status sync, or main merge. [Queue guide](docs/QUEUE_GUIDE.md) describes
recovery, evidence and controlled future-plan amendments. Historical changelog
entries stay historical; the new progress ledger records new completed work.
