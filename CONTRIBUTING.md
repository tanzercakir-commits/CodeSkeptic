...
The bounded parser-fuzz build and the reproducible local/CI campaign commands
are documented in [docs/fuzzing.md](docs/fuzzing.md).
...
Start each `phase-*` branch by running:

```bash
bash scripts/check_docs_sync.sh --fix
```

This appends newly merged protected-main task-closure receipts to the generated,
append-only [progress ledger](docs/PROGRESS.md) and regenerates the complete
open-work [TODO](docs/TODO.md) from the fixed task catalog in
[PLAN](docs/PLAN.md). Never edit TODO or PROGRESS by hand. Work only on a
`phase-*` branch; the sync command rejects direct `main` use.

A task closes only when the final protected-main commit message contains its
exact trailer, for example:

```text
Closes-CodeSkeptic-Task: CS-P10-01
```

A phase-branch trailer, local test output, changelog prose, or AI statement is
not completion authority. With squash merge, the trailer must be present in
the final squash commit message. The next phase branch reconciles that main
fact: PROGRESS receives the commit/task receipt and TODO automatically drops
the closed item. Legacy PROGRESS bytes remain frozen; ledger v2 records only
task-closing protected-main commits. An ordinary reconciliation merge has no
ledger entry. Therefore, after the final task-closing merge, one trailer-free
`phase-*` reconciliation can produce an empty TODO and a current ledger without
an infinite self-recording chain. The read-only docs-sync gate reconstructs
both generated files and fails on stale, manually changed, rewritten, missing,
unknown-task, dependency-invalid, non-ancestor, or non-`phase-*` work state.
Detailed rationale and measurement receipts continue to belong in
[the changelog](docs/devlog/changelog.md).
